...
function displayAllocationResults(result) {
    ...
    result.products.forEach(product => {
        const salesKg = product.sales_kg || product.quantity || 0;
        const copPerKg = salesKg > 0 ? (product.total_cost / salesKg) : 0;
...
function formatPoolCostLine(cost) {
    ...
    const poolRate = cost.pool_per_kg > 0
        ? `rate ₹${formatNumber(cost.pool_per_kg)}/kg`
        : '';
    const den = cost.allocation_denominator_kg > 0
        ? `denom ${formatNumber(cost.allocation_denominator_kg)} kg`
        : '';
    const productKg = cost.product_kg > 0
        ? `this line ${formatNumber(cost.product_kg)} kg`
        : '';
    const meta = [poolTot, poolRate, den, productKg].filter(Boolean).join(' · ');
    return `₹${formatNumber(cost.amount)} | ₹${formatNumber(cost.amount_per_kg || 0)}/kg` +
